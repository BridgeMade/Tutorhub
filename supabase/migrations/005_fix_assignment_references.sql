-- Fix assignment table to reference profiles directly instead of students/tutors tables
-- This is simpler since we have role information in profiles

-- Drop existing table and recreate with proper references
DROP TABLE IF EXISTS tutor_student_assignments;

-- Create tutor_student_assignments table with profile references
CREATE TABLE tutor_student_assignments (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  tutor_profile_id UUID REFERENCES profiles(id) ON DELETE CASCADE NOT NULL,
  student_profile_id UUID REFERENCES profiles(id) ON DELETE CASCADE NOT NULL,
  ...
  UNIQUE(tutor_profile_id, student_profile_id, subject_id),
  
  -- Add constraints to ensure roles are correct
  CONSTRAINT check_tutor_role CHECK (
    EXISTS (SELECT 1 FROM profiles WHERE id = tutor_profile_id AND role = 'tutor')
  ),
  CONSTRAINT check_student_role CHECK (
    EXISTS (SELECT 1 FROM profiles WHERE id = student_profile_id AND role = 'student')
  )
);
...
CREATE POLICY "Tutors can view own assignments" ON tutor_student_assignments
  FOR SELECT USING (auth.uid() = tutor_profile_id);
...
CREATE INDEX idx_tutor_student_assignments_tutor_profile_id ON tutor_student_assignments(tutor_profile_id);
CREATE INDEX idx_tutor_student_assignments_student_profile_id ON tutor_student_assignments(student_profile_id);
